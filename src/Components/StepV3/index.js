...
export default class StepV3 extends Component {
    ...
    onSubmit = async () => {
        ...
        fetch(API_URL+`/step/doneV3/${step._id}`, {
            method: 'POST',
            headers: await GET_TOKEN_HEADER(),
        }).then(res => res.json())
        .then(res => {
     ...
     if(step.type == 'PR')
        return (
            <>
                <Heading {...step} />
                <div className="my-5">
                    <PRBlockV3 firstTime project={project}  session={project.firstSession} onDelete={this.removeStepInfo} error={error} info={info} loading={this.state.loading} onPRChange={this.onPRChange} onPRSubmit={this.onPRSubmit} step={step} />
                </div>
            </>
            )
    ...
    if(step.type == 'ISSUE_SELECT' || step.type == 'ISSUE' || step.type == 'FPR-ISSUE'  )
        return (
            <>
                <Heading {...step} />
                <div className='my-5'>
                    <FPRIssueList project={project} loading={this.state.loading} step={step} onIssueAdded={this.onIssueAdded} />
                </div>
            </>
        )
    ...
    if(step.type == 'FORK')
        return (
            <>
                <Heading {...step} />
                <div className='my-5'>
                    <ForkStep project={project} loading={this.state.loading} step={step} />
                </div>
            </>
        )
    ...
    if(step.type == 'CLONE')
        return (
            <>
                <Heading {...step} />
                <div className='my-5'>
                    <CloneStep project={project} loading={this.state.loading} step={step} />
                </div>
            </>
        )
