...
export default class Lesson extends Component {
    ...
    fetchLessonData = async (slug, sessionId) => {
        return fetch(API_URL + `/lesson/slug/${slug}?sessionId=${sessionId}`, {
    ...
    onNextLesson = async (nextStep) => {
        ...
        fetch(API_URL+`/lesson/done/${this.props.session._id}/${this.state.lesson._id}`, {
            method:'POST',
            headers: await GET_TOKEN_HEADER()
        }).then(res => res.json())
        .then(res => {
    ...
    render() {
        ...
        return (
            <Card className="p-6">
                ...
                <div className='mt-4'>
                    { 
                        lesson.steps.sort((a,b)=> a.order - b.order).map(step => <Step onDone={this.onStepDone} step={step} session={this.props.session} />)
                    }
                </div>
                <div className="mt-6 flex justify-between items-center">
                    ...
                    <div>
                      ...
                      <div className={` ${!allStepsDone ? 'opacity-25 pointer-events-none' : ''} `}>
                        { nextStep ?   
                        <div onClick={() => this.onNextLesson(nextStep) } className="py-2 px-6 rounded text-xs bg-blue-500 text-white font-semibold cursor-pointer hover:bg-blue-600">Next Lesson</div> : 
                        <button onClick={() => this.onNextLesson()} className="py-2 px-6 rounded text-xs bg-blue-500 text-white font-semibold  cursor-pointer hover:bg-blue-600">Next Course</button>
                        }
                        </div>
                    </div>
                </div>
